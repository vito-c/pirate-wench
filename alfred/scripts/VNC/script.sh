...
#Get aliases associated with ips and names
aliasesFile=$(echo "`pwd`/aliases.txt")

# use an ip like a boass
if [[ "$(echo $COMMAND | awk -F"\." ' $0 ~ /^([0-9]{1,3}\.){3}[0-9]{1,3}$/ && $1 <=255 && $2 <= 255 && $3 <= 255 && $4 <= 255 ')" ]] 
then 
osascript <<EOS
...
# use aliases created by hand for now cause i'm a haxor
aliases=(); ips=(); 
while read line; do aliases+=("${line%%:*}"); ips+=(${line##*:}); done < "aliases.txt"
total="${#aliases[@]}"; 
for (( i=0; i<$total; i++ ))
do 
